...
UdpAgent::UdpAgent() : Agent(PT_UDP), seqno_(-1)
{
	...
	bind("totalRecvBytes_", &totalRecvBytes_);
}
...
void UdpAgent::recv(Packet* pkt, Handler*)
{
	...
	hdr_cmn* h = hdr_cmn::access(pkt);
	int recv_bytes = h->size() - 20;
	double sent_time = h->timestamp();
	double recv_time = Scheduler::instance().clock();
	totalRecvBytes_ = totalRecvBytes_ + recv_bytes;
	printf("UDP agent recieve packet %i\n",totalRecvBytes_);
	char out[100];
    sprintf(out, "record_msgdelay %i %f %f %f", h->uid(),sent_time,recv_time,recv_time-sent_time);
    Tcl& tcl = Tcl::instance();
    tcl.eval(out);
	Packet::free(pkt);
}

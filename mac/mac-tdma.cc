...
void MacTdma::recv(Packet* p, Handler* h) {
	...
	printf("MAC in %d receive packet %i size %d\n",index_,ch->uid(),ch->size());
	}
	//if(ch->direction() == hdr_cmn::UP) {
	//printf("MAC in %d receive packet %i size %d from phy error %d\n",index_,ch->uid(),ch->size(),ch->error());
	//}
...
void MacTdma::recvDATA(Packet *p){
	...
	printf("MAC in %d send packet %i size %d to LL\n",index_,ch->uid(),ch->size());
...
void MacTdma::send() 
{
	...
	printf("%d send packet %i in slot %d error %d\n",index_,ch->uid(),slot_count_,ch->error());
